...
public class Theme extends FactSource.FileFactSource implements Comparable<Theme> {
    ...
    public Theme(String name, String description, ThemeGroup group) {
        ...
        if (name2theme.containsKey(name))
            throw new IllegalArgumentException("Duplicate Theme: " + name);
    ...
    public static synchronized Theme getOrCreate(String name, String description, ThemeGroup group) {
        ...
        if (result == null)
            result = new Theme(name, description, group);
        return (result);
    }
    ...
    public File findFileInFolder(File folder) {
        ...
        if (tsv.exists()) {
            if (ttl.exists())
                Announce.warning("Theme", this, "exists as ttl and tsv in", folder, ". Using tsv.");
            return (tsv);
        }
        if (ttl.exists())
            return (ttl);
    ...
    public void close() throws IOException {
        if (factWriter == null)
            throw new IOException("Theme " + this + " cannot be closed because it was not open");
    ...
    public synchronized Theme assignToFolder(File folder) throws IOException {
        ...
        if (file != null) {
            if (file.equals(f))
                ...
                throw new IOException("Theme " + this + " is already assigned to file " + file + ", cannot assign it to " + f);
        }
        file = f;
        cache = null;
        return (this);
    ...
    public synchronized FactCollection factCollection() throws IOException {
        if (factWriter != null)
            throw new IOException("Theme " + this + " is currently being written");
        if (file == null)
    ...
    public Map<String, String> dictionary() throws IOException {
        ...
        return (new Map<String, String>() {
            ...
            public boolean containsKey(Object arg0) {
                if (cache == null)
                    throw new RuntimeException("Cache of " + this + " was killed");
            ...
            public String get(Object arg0) {
                ...
                /*
                 * Map<String, List<Fact>> map = cache.index.get(arg0); if (map == null) return
                 * null; List<Fact> objects = map.get(YAGO.hasTranslation); if (objects == null
                 * || objects.isEmpty()) return (null); return (objects.get(0).getObject());
                 */
            }
         ...
         * Announce.doing("Loading dictionary", this); for (Fact f : this) { if
         * (!f.getRelation().equals("<_hasTranslation>")) continue;
         * result.put(f.getSubject(), f.getObject()); } Announce.done(); return
         * (result);
         */
    }
