...
public class PersonNameExtractor extends Extractor {
    ...
    public void extract() throws Exception {
        ...
        for (Fact f : TransitiveTypeSubgraphExtractor.YAGOTRANSITIVETYPE) {
            ...
            if (!given.endsWith(".") && given.length() > 1) {
                write(PERSONNAMES, new Fact(f.getArg(1), "<hasGivenName>", FactComponent.forStringWithLanguage(given, lang)), PERSONNAMESOURCES,
                        source, "PersonNameExtractor");
    ...
    private void writeNormalized(String entity, String name, String source) throws IOException {
        ...
        if (!normalizedName.equals(name)) {
            write(PERSONNAMEHEURISTICS, new Fact(entity, RDFS.label, FactComponent.forStringWithLanguage(normalizedName, "eng")), PERSONNAMESOURCES,
                    source, "PersonNameExtractor_normalized");
        }
    }
