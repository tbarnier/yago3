...
public class GenderNameExtractor extends Extractor {
    ...
    protected static String gender(Map<String, int[]> givenName2gender, String givenName) {
        ...
        if (gender == null)
            return (null);
        if (FrequencyVector.wilson(gender[0] + gender[1], gender[0])[0] > 0.95)
            return ("<male>");
        if (FrequencyVector.wilson(gender[0] + gender[1], gender[1])[0] > 0.95)
            return ("<female>");
        return (null);
    }
    ...
    private void registerGender(Map<String, int[]> firstName2gender, String subject, String gender) {
        ...
        if (counters == null)
            firstName2gender.put(givenName, counters = new int[2]);
    ...
    public void extract() throws Exception {
        ...
        for (Theme t : categoryMembers) {
            for (Fact f : t) {
                ...
                String gender = null;
                if (male.reset(category).matches())
                    gender = "<male>";
                else
                    if (female.reset(category).matches())
                        gender = "<female>";
                if (gender != null) {
        ...
        for (Fact f : TransitiveTypeSubgraphExtractor.YAGOTRANSITIVETYPE) {
            ...
            if (!lastEntity.equals(f.getSubject())) {
                if (isPerson) {
                    if (gender == null)
                        gender = entityToGender.get(lastEntity);
                    if (gender != null) {
            ...
            String category = f.getObject();
            if (category.equals(YAGO.person))
                isPerson = true;
            else
                if (male.reset(category).matches())
                    gender = "<male>";
        ...
        for (String givenName : givenName2gender.keySet()) {
            ...
            if (gender != null)
                GENDERNAMES.write(new Fact(FactComponent.forString(givenName), "<hasGender>", gender));
        }
        ...
        for (Fact f : TransitiveTypeSubgraphExtractor.YAGOTRANSITIVETYPE) {
            if (f.getRelation().equals(RDFS.type) && f.getObject().equals(YAGO.person)) {
                ...
                if (givenName != null) {
                    gender = gender(givenName2gender, givenName);
                    if (gender != null) {
                        write(GENDERSBYNAME, new Fact(f.getSubject(), "<hasGender>", gender), GENDERSOURCES, source,
                                "Gender derived from given name.");
                    }
                }
            }
        }
