...
public class MentionLinkLikelihoodExtractor extends MultilingualWikipediaExtractor {
    ...
    private void loadMentionsForLanguage() throws IOException {
        Collection<Fact> fs = DisambiguationPageExtractor.DISAMBIGUATIONMEANSFACTS.inLanguage(language).factCollection()
                .getFactsWithRelation(RDFS.label);
        addFacts(fs);
            ...
            switch (FileLines.findIgnoreCase(in, "<title>")) {
                case -1:
                    ...
                    for (String mentionToken : mentionTokensLinkCount.keySet()) {
                        ...
                        Fact f = new Fact(FactComponent.forStringWithLanguage(mentionToken, language), "<_hasLinkLikelihood>",
                ...
                case 0:
                    ...
                    if (pagesProcessed % 100_000 == 0) {
                        Announce.debug(NumberFormatter.ISOtime() + " - MentionLinkLikelihoodExtractor(" + language + "): " + pagesProcessed
                                + " pages Processed.");
                    }
