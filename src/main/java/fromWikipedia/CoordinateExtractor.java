...
public class CoordinateExtractor extends EnglishWikipediaExtractor {
    ...
    public CoordinateExtractor(File wikipedia) throws IOException {
        super(wikipedia);
    }
    ...
    public static final Pattern coordPattern1 = Pattern
            .compile("(?i)coord" + bar + integer + bar + integer + bar + flote + bar + dir + bar + integer + bar + integer + bar + flote + bar + dir);

    /** {{Coord|44.112|N|87.913|W|display=title}} */
    public static final Pattern coordPattern2 = Pattern.compile("(?i)coord" + bar + flote + bar + dir + bar + flote + bar + dir);

    /** {{Coord|57|18|N|4|27|W|display=title}} */
    public static final Pattern coordPattern3 = Pattern
            .compile("(?i)coord" + bar + integer + bar + flote + bar + dir + bar + integer + bar + flote + bar + dir);

    /** {{Coord|44.112|-87.913|display=title}} */
    public static final Pattern coordPattern4 = Pattern.compile("(?i)coord" + bar + flote + bar + flote);

    protected void writeCoords(String entity, Double la, Double lo) throws IOException {
        if (la != null)
            write(COORDINATES, new Fact(entity, "<hasLatitude>", FactComponent.forDegree(la)), COORDINATE_SOURCES, FactComponent.wikipediaURL(entity),
                    "CoordinateExtractor");
        if (lo != null)
            write(COORDINATES, new Fact(entity, "<hasLongitude>", FactComponent.forDegree(lo)), COORDINATE_SOURCES,
    ...
    public static Double getCoord(String degree, String minute, String second, String eastWestNorthSouth) {
        int sign = 1;
        if (!eastWestNorthSouth.isEmpty() && "SswW".indexOf(eastWestNorthSouth.charAt(0)) != -1)
            sign = -1;
        try {
        ...
        while (true) {
            switch (FileLines.findIgnoreCase(in, "<title>", "{{coord", "{{ coord")) {
                ...
                case 2:
                    ...
                    if (m.find()) {
                        writeCoords(titleEntity, getCoord(m.group(1), m.group(2), m.group(3), m.group(4)),
                                getCoord(m.group(5), m.group(6), m.group(7), m.group(8)));
                        break;
                    }
                    m = coordPattern2.matcher(val);
                    if (m.find()) {
                        writeCoords(titleEntity, getCoord(m.group(1), "0", "0", m.group(2)), getCoord(m.group(3), "0", "0", m.group(4)));
                        break;
                    }
                    m = coordPattern3.matcher(val);
                    if (m.find()) {
                        writeCoords(titleEntity, getCoord(m.group(1), m.group(2), "0", m.group(3)),
                                getCoord(m.group(4), m.group(5), "0", m.group(6)));
                        break;
                    }
                    m = coordPattern4.matcher(val);
                    if (m.find()) {
                        writeCoords(titleEntity, getCoord(m.group(1), "0", "0", "N"), getCoord(m.group(2), "0", "0", "E"));
                        break;
                    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        new CoordinateExtractor(new File("/san/jbiega/multiWikiSymlinks/en_wiki-20121201-pages-articles.xml"))
