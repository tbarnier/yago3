...
public class WikidataImageLicenseExtractor extends DataExtractor {
    ...
    public static final Theme WIKIDATAIMAGELICENSE = new Theme("wikidataImageLicenses", "Licences extracted for wikidata Images");
    ...
    public WikidataImageLicenseExtractor(File input) throws IOException {
        super(input);
    }

    public WikidataImageLicenseExtractor() throws IOException {
    ...
    private static Pattern authorFieldPattern = Pattern
            .compile("\\|\\s*(?:author|artist|creator)\\s*=\\s*(.+?)(?!\\{\\{.*?\\|.*?\\}\\})(?:[^a-zA-Z]\\|)", Pattern.CASE_INSENSITIVE);
    ...
    static {
        ...
        tempMap.put("wikiLanguageUser",
                Pattern.compile("\\[\\[(?::?(.{2,3}):)?((?!:File:)(?:user:)?.+?)(?:\\|(.+?))?\\]\\]", Pattern.CASE_INSENSITIVE));
        ...
        tempMap.put("wikiUserAtProject",
                Pattern.compile("\\{\\{(?:(?:user at project)|(?:original uploader))\\|(.*?)\\|(?:wikipedia\\|)?(.{2,3})\\}\\}"));
    ...
    static {
        ...
        tempMap.put("CreativeCommonsLicense", Pattern.compile(
                "(?:\\{\\{|\\|)cc-(by(?:-sa|-nd|-nc|-nc-sa|-nc-nd|))-(\\d\\.\\d|all)(?:-(.{2,3}))?(?:\\}\\}|\\|)", Pattern.CASE_INSENSITIVE));
    ...
    static {
        ...
        for (String type : CC_TYPES)
            for (String version : CC_VERSIONS)
                tempMap.put("CreativeCommonsLicense-" + type + "-" + version,
                        "https://creativecommons.org/licenses/" + type.toLowerCase() + "/" + version);
    ...
    public void extract() throws Exception {
        ...
        while (FileLines.findIgnoreCase(in, "<title>File:") != -1) {
            ...
            if (imageFileName != null && imageUrlByName.containsKey(imageFileName)) {
                ...
                if (trademark) {
                    WIKIDATAIMAGELICENSE
                            .write(new Fact(FactComponent.forUri(imageUrl), YAGO.hasTrademark, FactComponent.forYagoEntity(trademark.toString())));
                }
    ...
    private Map<String, String> getFileNames() throws IOException {
        ...
        for (Fact f : entityImages) {
            ...
            String imageWikiPageUrl = FactComponent
                    .stripBrackets(WikidataImageExtractor.WIKIDATAIMAGES.factCollection().getObject(imageID, YAGO.hasWikiPage));
        ...
        for (String key : imageLicensePatterns.keySet()) {
            ...
            while (licenseMatcher.find()) {
                ...
                // If it is GFDL, check if it has a specific version.
                    ...
                    if (key.equals("GNUFreeDocumentationLicense")) {
                    ...
                    // If it is GPL, check for versions.
                        ...
                        if (key.equals("GNUGeneralPublicLicense")) {
                        ...
                        // If it is Creative Common and does not have a specific language (It is still in the hard coded licenses with different key):
                            ...
                            if (key.equals("CreativeCommonsLicense") && licenseMatcher.group(3) == null) {
                            ...
                            // It is not in the hard coded licenses:
                                ...
                                switch (key) {
                                    case "CreativeCommonsLicense":
                                        if (licenseMatcher.group(2).equals("all")) {
                                            for (String version : CC_VERSIONS) {
                                                url = hardCodedLicenses.get(key + "-" + licenseMatcher.group(1).toUpperCase() + "-" + version) + "/"
                                                        + licenseMatcher.group(3);
                                                licenses.addedLicenses.put(licenseID, url);
                                            }
                                        }
                                        else {
                                            url = hardCodedLicenses
                                                    .get(key + "-" + licenseMatcher.group(1).toUpperCase() + "-" + licenseMatcher.group(2)) + "/"
                                                    + licenseMatcher.group(3);
                                            licenses.addedLicenses.put(licenseID, url);
                                        }
                                        break;
                                    ...
                                    case "OpenGovernmentLicence":
                                        url = "https://www.nationalarchives.gov.uk/doc/open-government-licence/version/"
                                                + ((licenseMatcher.group(1) == null) ? "1" : licenseMatcher.group(1));
                                        licenses.addedLicenses.put(licenseID, url);
                                        break;
                                }
                            }
                    ...
                    switch (key) {
                        ...
                        case "wikiLanguageUser":
                            if (authorTypeMatcher.group(1) != null) {
                                author.url = "http://" + authorTypeMatcher.group(1) + "." + wikipediaUrl
                                        + authorTypeMatcher.group(2).replaceAll(" ", "_");
                            }
                            else {
                        ...
                        case "wikiUserAtProject":
                            author.url = "http://" + authorTypeMatcher.group(2) + "." + wikipediaUrl + "User:"
                                    + authorTypeMatcher.group(1).replaceAll(" ", "_");
                            break;
