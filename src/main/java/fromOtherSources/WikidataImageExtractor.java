...
public class WikidataImageExtractor extends DataExtractor {
    ...
    public static final Theme WIKIDATAIMAGES = new Theme("wikidataImages", "Images in wikidata dump for entities");

    public static final Theme WIKIDATAIMAGESNEEDSTYPECHECK = new Theme("wikidataImagesNeedsTypeCheck", "Images in wikidata dump for entities");
    ...
    public WikidataImageExtractor(File wikidata) throws IOException {
        super(wikidata);
    }

    public WikidataImageExtractor() throws IOException {
    ...
    public void extract() throws Exception {
        ...
        while (nr.hasNext()) {
            ...
            if (f.getObject().endsWith("#Item>")) {
                if (!images.isEmpty()) {
                    String image = Char17.decodePercentage(FactComponent.stripBrackets(pickImage(yagoEntityMostEnglish, images))).replaceAll(" ",
                            "_");
                ...
                yagoEntityMostEnglish = getMostEnglishEntityName(
                        reverseWikidataInstances.getFactsWithSubjectAndRelation(f.getSubject(), RDFS.sameas));
                prevImage = null;
            }
            ...
            // example: <some random id> <https://www.wikidata.org/wiki/Property:P18> <filename.jpg> .
                ...
                if (f.getObject().matches(".*commons\\.wikimedia.*\\.(jpg|jpeg|png|svg|tif|tiff|gif)>$") && yagoEntityMostEnglish != null) {
                ...
                // example : <random id> <http://www.wikidata.org/ontology#rank> <http://www.wikidata.org/ontology#NormalRank> . 
                    ...
                    if (prevImage != null && prevImage.getSubject().equals(f.getSubject()) && f.getRelation().endsWith("#rank>")) {
