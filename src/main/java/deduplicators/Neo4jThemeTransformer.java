...
public class Neo4jThemeTransformer extends Extractor {
    ...
    public static final Theme NEO4JDONE = new Theme("neo4j_extractor_done",
            "This is a dump theme created by Neo4j Extractor to ensure that the extractor is done and allow reusing it.");
    ...
    public void extract() throws Exception {
        ...
        tempNodes.clear();
        writeToFile(hasWikipediaUrlRelationsFileName, new String[] { ":START_ID(WikidataInstance)", ":END_ID(WikipediaUrl)" }, tempRelations);
        tempRelations.clear();
        ...
        D.p("Finishing " + wikipeidiaUrlNodesFileName + " " + hasWikipediaUrlRelationsFileName + (System.currentTimeMillis() - startTimeFileMaking));
        ...
        writeToFile(wikidataImageNodesFileName, new String[] { "wikiPage:ID(WikidataImage)", "imageUrl", "hasTrademark" }, tempNodes);
        tempNodes.clear();
        writeToFile(hasImageRelationsFileName, new String[] { ":START_ID(WikidataInstance)", ":END_ID(WikidataImage)" }, tempRelations);
        tempRelations.clear();

        addNodesToCommand("WikidataImage", wikidataImageNodesFileName);
        addRelationsToCommand("hasImage", hasImageRelationsFileName);

        D.p("Finishing " + wikidataImageNodesFileName + " " + hasImageRelationsFileName + (System.currentTimeMillis() - startTimeFileMaking));
        ...
        writeToFile(wikidataImageLicenseNodesFileName, new String[] { "name:ID(WikidataImageLicense)", "url" }, tempNodes);
        tempNodes.clear();
        writeToFile(hasLicenseRelationsFileName, new String[] { ":START_ID(WikidataImage)", ":END_ID(WikidataImageLicense)" }, tempRelations);
        tempRelations.clear();
        ...
        D.p("Finishing " + wikidataImageLicenseNodesFileName + " " + hasLicenseRelationsFileName
        ...
        tempNodes.clear();
        writeToFile(hasAuthorRelationsFileName, new String[] { ":START_ID(WikidataImage)", ":END_ID(Author)" }, tempRelations);
        tempRelations.clear();
        ...
        tempNodes.clear();
        writeToFile(hasOTRSPermissionRelationsFileName, new String[] { ":START_ID(WikidataImage)", ":END_ID(OTRSPermission)" }, tempRelations);
        tempRelations.clear();
        ...
        D.p("Finishing " + OTRSPermissionNodesFileName + " " + hasOTRSPermissionRelationsFileName
                + (System.currentTimeMillis() - startTimeFileMaking));
        ...
        for (String entity : WikidataEntityGeoCoordinateExtractor.WIKIDATAENTITYGEOCOORDINATES.factCollection().getSubjects()) {
            if (entity.startsWith("<yagoTheme_"))
                continue;
            entity_wikidataId.putIfAbsent(entity, entity);

            String latitude = FactComponent.stripQuotes(FactComponent.getString(
                    WikidataEntityGeoCoordinateExtractor.WIKIDATAENTITYGEOCOORDINATES.factCollection().getObject(entity, YAGO.hasLatitude)));
            String longitude = FactComponent.stripQuotes(FactComponent.getString(
                    WikidataEntityGeoCoordinateExtractor.WIKIDATAENTITYGEOCOORDINATES.factCollection().getObject(entity, YAGO.hasLongitude)));
        ...
        writeToFile(geoLocationNodesFileName, new String[] { "location_id:ID(Location)", "latitude:float", "longitude:float" }, tempNodes);
        tempNodes.clear();
        writeToFile(hasGeoLocationRelationsFileName, new String[] { ":START_ID(WikidataInstance)", ":END_ID(Location)" }, tempRelations);
        tempRelations.clear();

        addNodesToCommand("Location", geoLocationNodesFileName);
        addRelationsToCommand("hasGeoLocation", hasGeoLocationRelationsFileName);

        D.p("Finishing " + geoLocationNodesFileName + " " + hasGeoLocationRelationsFileName + (System.currentTimeMillis() - startTimeFileMaking));

        // Adding Types:
        D.p("Starting " + typeNodesFileName + " " + hasTypeRelationsFileName + " " + isSubclassOfRelationsFileName);
        startTimeFileMaking = System.currentTimeMillis();
        ...
        writeToFile(hasTypeRelationsFileName, new String[] { ":START_ID(WikidataInstance)", ":END_ID(Type)" }, tempRelations);
        tempRelations.clear();
        ...
        writeToFile(isSubclassOfRelationsFileName, new String[] { ":START_ID(Type)", ":END_ID(Type)" }, tempRelations);
        tempRelations.clear();
        ...
        writeToFile(typeHasTranslationRelationsFileName, new String[] { ":START_ID(Type)", ":END_ID(Type)" }, tempRelations);
        tempRelations.clear();
        ...
        D.p("Finishing " + typeNodesFileName + " " + hasTypeRelationsFileName + " " + isSubclassOfRelationsFileName + " "
                + (System.currentTimeMillis() - startTimeFileMaking));
        ...
        for (Theme theme : RedirectExtractor.REDIRECTFACTS.inLanguages(MultilingualExtractor.wikipediaLanguages)) {
            ...
            for (Fact f : theme.factCollection().getFactsWithRelation("<redirectedFrom>")) {
                ...
                wikidataId_properies.computeIfAbsent(entity_wikidataId.get(entity), k -> new wikidataInstanceProperties()).redirectedFrom
                        .add(f.getObject());
            }
            theme.killCache();
            D.p("Finishing " + theme.name + (System.currentTimeMillis() - startTime));
        ...
        for (Theme theme : ConteXtExtractor.CONTEXTFACTS.inLanguages(MultilingualExtractor.wikipediaLanguages)) {
            ...
            for (Fact f : theme.factCollection().getFactsWithRelation("<hasCitationTitle>")) {
                ...
                wikidataId_properies.computeIfAbsent(entity_wikidataId.get(entity), k -> new wikidataInstanceProperties()).citationTitles
                        .add(f.getObject());
            }
            D.p("Finishing " + theme.name + (System.currentTimeMillis() - startTime));
        }
        ...
        for (Fact f : HardExtractor.HARDWIREDFACTS.factCollection().getFactsWithRelation(RDFS.label)) {
            if (entity_wikidataId.containsKey(f.getSubject())) {
                wikidataId_properies.computeIfAbsent(entity_wikidataId.get(f.getSubject()), k -> new wikidataInstanceProperties()).labels
                        .add(f.getObject());
            }
        }
        ...
        writeToFile(entityHasTranslationRelationsFileName, new String[] { ":START_ID(Entity)", ":END_ID(Entity)" }, tempRelations);
        tempRelations.clear();
        ...
        tempNodes.clear();
        writeToFile(sameAsRelationsFileName, new String[] { ":START_ID(WikidataInstance)", ":END_ID(Entity)" }, tempRelations);
        tempRelations.clear();
        ...
        writeToFile(metaInformationFileName,
                new String[] {
                        "confName:ID(Meta)",
                        "KB_creationDate",
                        "languages:string[]",
                        "KB_WikipediaSources:string[]",
                        "collection_size",
                        "datasource_YAGO3",
                        "creationDate" },
                tempNodes);
