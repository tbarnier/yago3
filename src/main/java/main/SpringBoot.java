...
public class SpringBoot {
    ...
    public static synchronized void callNext(Extractor finished, boolean success) {
        log.info(NumberFormatter.ISOtime());
        if (finished != null) {
        ...
        for (int i = 0; i < extractorsToDo.size(); i++) {
            if (extractorsRunning.size() >= conf.getNumThreads())
                break;
        ...
        // Print new state
        log.info("Extractors running:", extractorsRunning);
        log.warn("Extractors failed:", extractorsFailed);
        if (!extractorsRunning.isEmpty())
            return;

        // In case we finished print summary
        long now = System.currentTimeMillis();
        log.info("Finished at", NumberFormatter.ISOtime());
        log.info("Time needed:", NumberFormatter.formatMS(now - startTime));
    ...
    /**
     * Returns a topological sort. If map contains a -> [b, c], b -> [c, d], it may
     * return c, d, b, a
     */
    ...
    public static void printNeededExtractorsForThemes(Set<Theme> themes) {
        ...
        while (!stillMissing.isEmpty()) {
            ...
            if (toRun == null) {
                ...
                if (!extractorsToRun.contains(toRun)) {
    ...
    public static String tail(File src, int maxLines) throws IOException {
        ...
        try (RandomAccessFile f = new RandomAccessFile(src, "r")) {
            ...
            for (p = length - 1; p >= 0 && lines < maxLines; p--) {
                ...
                if (b == 10 || p == 0) {
                    if (p < length - 1) {
                        if (sb.length() > 0 && lines < maxLines - 1 && p != 0)
                            sb.append('\n');
                        lines++;
                    }
                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String lockFile = conf.getOutputFolder().getAbsolutePath() + "/yago.lock";

        if (!lock(lockFile)) {
            log.info("another instance of YAGO seems to be running. (Otherwise delete {})", lockFile);
            System.exit(-1);
        }
        ...
        if (conf.isReuse()) {
            ...
            for (Theme t : Theme.all()) {
                File f = t.findFileInFolder(conf.getOutputFolder());
            ...
            if (conf.isRerunDependentExtractors()) {
                for (Extractor e : extractorsToDo) {
            ...
            log.info("Reusing existing themes");
            for (Theme t : Theme.all()) {
                if (!available.contains(t)) {
                    log.error(" Not found:", t);
                }
                else {
                    if (reusable.contains(t)) {
                        t.assignToFolder(conf.getOutputFolder());
                        log.info(" Reusing:", t);
                        themesWeHave.add(t);
                    }
                    else {
                        log.info(" Overwriting:", t);
                    }
                }
            }
        }
    ...
    public static List<Extractor> extractors(List<String> extractorNames) {
        ...
        for (String extractorName : extractorNames) {
            if (extractorName.trim().startsWith("#"))
                continue;
    ...
    public static List<Extractor> extractorsForCall(String extractorCall) {
        return call2extractor.computeIfAbsent(extractorCall, k -> {
            ...
            if (superclasses.contains(MultilingualWikipediaExtractor.class)) {
                for (String language : conf.getWikipedias().keySet()) {
                    extractors.add(MultilingualWikipediaExtractor.forName((Class<MultilingualWikipediaExtractor>) clss, language,
                            conf.getWikipedias().get(language)));
                }
            }
            else
                if (superclasses.contains(MultilingualExtractor.class)) {
                    for (String language : conf.getWikipedias().keySet()) {
                    ...
                    if (superclasses.contains(EnglishWikipediaExtractor.class)) {
                        ...
                        if (superclasses.contains(DataExtractor.class)) {
                            ...
                            if (superclasses.contains(Neo4jThemeTransformer.class)) {
                                if (conf.getNeo4jFolder() != null) {
                                    extractors.add(new Neo4jThemeTransformer(conf.getNeo4jFolder().getAbsolutePath()));
                                }
                                else {
